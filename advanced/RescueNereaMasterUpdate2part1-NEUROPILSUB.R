...
dir<-"C:/Users/nllamosa/Desktop/Exp" ; for (i in dir) { setwd(i)
        setwd(i)}
dir<-"Z:/Nerea/2 photon imaging/Cre-Rum2-GcaMP6" ; for (i in dir) { setwd(i)
        setwd(i)}

# list and select the files to be working on
files<-list.files(pattern='.mat', recursive = TRUE)
files<-files[grep("analysis-", files)]
#toExclude<-c(770) # files to exclude based on error in length (time smasterData)
...
# Neuropil manipulation for substration
tempNeuropil=masterData[rowName==1, c('value', 'timeStamp', 'sID','whiskImage','whiskStim','depth','session','date','stimParam')]
fwrite(tempNeuropil, "Z:/Nerea/2 photon imaging/Cre-Rum2-GcaMP6/OldNeuropil_Cre-Rum2.csv" )
colnames(tempNeuropil)[colnames(tempNeuropil)=="value"] <- "Neuropilvalue"
masterData=merge(masterData, tempNeuropil, by=c('timeStamp', 'sID','whiskImage','whiskStim','depth','session','date','stimParam'), all=TRUE)
masterData[, NeuropilSubvalue := masterData$value-masterData$Neuropilvalue]
...
tempNeuropil<-merge(tempNeuropil, stimID, by=c('timeStamp','stimParam'), all=TRUE)
...
#geno<-c('wt')
#sID<-unique(masterDataN$sID)
#sIDgeno<-data.table(geno,sID)
#masterDataN<-merge(masterDataN, sIDgeno, by='sID')

#tempNeuropil<-merge(tempNeuropil, sIDgeno, by='sID')

# write master file in the working directory
#fwrite(masterDataN, 'NereaMASTERupdated_Neuropil.csv')
#fwrite(masterDataN, 'NereaMASTERupdated_NeuropilGavinHypo.csv')

#fwrite(tempNeuropil[stimParam=='five'], 'NereaMASTERupdated_NeuropilONLY-five.csv')
#fwrite(tempNeuropil[stimParam=='ten'], 'NereaMASTERupdated_NeuropilONLY-ten.csv')
#fwrite(tempNeuropil[stimParam=='spont'], 'NereaMASTERupdated_NeuropilONLY-spont.csv')
