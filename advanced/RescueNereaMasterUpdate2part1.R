...
dir<-"Z:/Nerea/2 photon imaging/new baselines/MatLab analysis 2nd cohort" ; for (i in dir) { setwd(i)
        setwd(i)}
dir<-"/Volumes/MillerRumbaughLab/Nerea/2 photon imaging/new baselines/MatLab analysis 2nd cohort" ; for (i in dir) { setwd(i)
        setwd(i)}
...
geno<-c('wt','het', 'het', 'wt', 'het', 'wt', 'het', 'het', 'wt', 'wt', 'wt')
sID<-unique(masterDataNew1$sID)
sIDgeno<-data.table(geno,sID)
masterDataN<-merge(masterDataNew, sIDgeno, by='sID')
